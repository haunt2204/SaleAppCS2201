...
def add_receipt(cart):
    if cart:
        ...
        for c in cart.values():
            d = ReceiptDetail(quantity=c['quantity'], unit_price=c['price'], product_id=c['id'], receipt=r)
            db.session.add(d)
...
def stats_revenue_by_period(year=datetime.now().year, period='month'):
    query = db.session.query(func.extract(period, Receipt.created_date), func.sum(ReceiptDetail.quantity*ReceiptDetail.unit_price))\
